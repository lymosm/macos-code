...
AsusFn* AsusFn::instance = nullptr;
AsusFn* AsusFn::getSharedInstance() {
    return instance;
}
void AsusFn::increaseBrightness() {
    IOLog("AsusFn::increaseBrightness called\n");
...
OSDefineMetaClassAndStructors(AsusFn, IOService)

bool AsusFn::init(OSDictionary *dict) {
    if (!super::init(dict)) {
        return false;
    }
    ...
    kev.setEventCode(AsusFnEventCode);
...
IOService *AsusFn::probe(IOService *provider, SInt32 *score) {
...
bool AsusFn::start(IOService *provider) {
    ...
    setProperty("Copyright", "Copyright © 2025 Tommy. All rights reserved.");

    extern kmod_info_t kmod_info;
    setProperty("AsusFn-Version", kmod_info.version);
#ifdef DEBUG
    setProperty("AsusFn-Build", "Debug");
#else
    setProperty("AsusFn-Build", "Release");
#endif
...
void AsusFn::stop(IOService *provider) {
    if(delaySource){
...
int AsusFn::wmi_parse_guid(const char *in, char *out) {
    for (int i = 3; i >= 0; i--) {
...
int AsusFn::wmi_evaluate_method(uint32_t method_id, uint32_t arg0, uint32_t arg1) {
    OSObject *params[3];
...
void AsusFn::initATKDevice() {
    wmi_evaluate_method(ASUS_WMI_METHODID_INIT, 0, 0);
}

void AsusFn::initALSDevice() {
...
void AsusFn::initVirtualKeyboard() {
...
bool AsusFn::refreshALS(bool post) {
    if (!alsDevice) {
        return false;
    }
...
void AsusFn::handleMessage(int code) {
    switch (code) {
...
void AsusFn::letSleep() {
    kev.sendMessage(kDaemonSleep, 0, 0);
}

void AsusFn::toggleAirplaneMode() {
...
void AsusFn::toggleALS(bool state) {
...
void AsusFn::toggleBatteryConservativeMode(bool state) {
    if (!isBatteryRSOCAvailable) {
...
int AsusFn::checkBacklightEntry() {
...
void AsusFn::readPanelBrightnessValue() {
...
IOReturn AsusFn::postKeyboardInputReport(const void *report, uint32_t reportSize) {
...
void AsusFn::dispatchCSMRReport(int code, int loop) {
...
void AsusFn::registerNotifications() {
    auto *key = OSSymbol::withCString(kDeliverNotifications);
    auto *propertyMatch = propertyMatching(key, kOSBooleanTrue);

    IOServiceMatchingNotificationHandler notificationHandler = OSMemberFunctionCast(IOServiceMatchingNotificationHandler, this, &AsusFn::notificationHandler);
...
void AsusFn::notificationHandlerGated(IOService *newService, IONotifier *notifier) {
...
bool AsusFn::notificationHandler(void *refCon, IOService *newService, IONotifier *notifier) {
    command_gate->runAction(OSMemberFunctionCast(IOCommandGate::Action, this, &AsusFn::notificationHandlerGated), newService, notifier);
    return true;
}

void AsusFn::dispatchMessageGated(int *message, void *data) {
...
void AsusFn::dispatchMessage(int message, void *data) {
    command_gate->runAction(OSMemberFunctionCast(IOCommandGate::Action, this, &AsusFn::dispatchMessageGated), &message, data);
}

void AsusFn::registerVSMC() {
...
bool AsusFn::vsmcNotificationHandler(void *sensors, void *refCon, IOService *vsmc, IONotifier *notifier) {
    if (sensors && vsmc) {
        DBGLOG("atk", "got vsmc notification");
        auto self = static_cast<AsusFn *>(sensors);
...
EXPORT extern "C" kern_return_t ADDPR(kern_start)(kmod_info_t *, void *) {
    ...
    ADDPR(debugEnabled) = checkKernelArgument("-vsmcdbg") || checkKernelArgument("-asusfndbg");
    return KERN_SUCCESS;
}
