// TommySyncTime_msgloop.c
// 可靠通知接收：专用 mach_msg() 线程 + IODataQueueAllocateNotificationPort
//
// 编译：clang -framework IOKit -framework CoreFoundation -o TommySyncTime TommySyncTime_msgloop.c
// 运行：sudo ./TommySyncTime
...
static void drain_queue(void) {
    ...
    while (IODataQueueDataAvailable(g_queue)) {
        ...
        if (kr != KERN_SUCCESS || size != sizeof(event)) {
            log_now("TommySyncTime", "Dequeue error: kr=0x%x size=%u", kr, size);
            break;
        }
        log_now("TommySyncTime", "Dequeued event %llu", (unsigned long long)event);
        if (event == 1) {
            log_now("TommySyncTime", "wake event -> syncing time");
            int ret = system("/usr/bin/sntp -sS ntp.aliyun.com");
            int ret2 = system("/usr/local/bin/active-audio");
            log_now("TommySyncTime", "sntp returned %d", ret);
        }
    }
}
...
static void cleanup(void) {
    log_now("TommySyncTime", "cleanup called");
    g_running = 0;
...
static void sig_handler(int sig) {
    (void)sig;
    log_now("TommySyncTime", "signal received, shutting down...");
    g_running = 0;
...
int main(void) {
    if (geteuid() != 0) {
        fprintf(stderr, "TommySyncTime: must run as root!\n");
        return 1;
    }
    ...
    log_now("TommySyncTime", "waiting for %s provider...", SERVICE_NAME);
    ...
    if (service == IO_OBJECT_NULL) {
        log_now("TommySyncTime", "%s provider not found after %d seconds", SERVICE_NAME, WAIT_SECONDS);
        return 1;
    }
    ...
    if (kr != KERN_SUCCESS) {
        log_now("TommySyncTime", "IOServiceOpen failed: 0x%x", kr);
        return 1;
    }
    log_now("TommySyncTime", "opened provider and obtained connection (user client)");

    // 映射共享队列
    kr = IOConnectMapMemory64(g_conn, 0, mach_task_self(), &g_queueAddr, &g_queueSize, kIOMapAnywhere);
    if (kr != KERN_SUCCESS) {
        log_now("TommySyncTime", "IOConnectMapMemory64 failed: 0x%x", kr);
        return 1;
    }
    g_queue = (IODataQueueMemory *)g_queueAddr;
    log_now("TommySyncTime", "queue mapped at %p, size=%llu",
    ...
    if (g_notifyPort == MACH_PORT_NULL) {
        log_now("TommySyncTime", "IODataQueueAllocateNotificationPort failed");
        return 1;
    }
    log_now("TommySyncTime", "notifyPort=0x%x allocated (IODataQueue helper)", g_notifyPort);

    // 把通知端口交给内核（selector/type 用 0；若你的 UserClient 另有 type 约定，保持一致）
    kr = IOConnectSetNotificationPort(g_conn, g_notifyPort, 0, 0);
    if (kr != KERN_SUCCESS) {
        log_now("TommySyncTime", "IOConnectSetNotificationPort failed: 0x%x", kr);
        return 1;
    }
    ...
    if (perr != 0) {
        log_now("TommySyncTime", "pthread_create failed: %d", perr);
        return 1;
    }

    log_now("TommySyncTime", "message loop thread started; press Ctrl+C to quit");
    // 主线程闲置。收到 SIGINT/SIGTERM 后会退出（pause 会被信号中断）
    while (g_running) pause();

    log_now("TommySyncTime", "main exiting");
    return 0;
}
