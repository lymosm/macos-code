...
IORegistryEntry* LymosTimer::getDeviceByAddress(IORegistryEntry *parent, UInt64 address, UInt64 mask) {
...
bool LymosTimer::start(IOService *provider) {
    ...
    if (propertyMatch) {
        IOServiceMatchingNotificationHandler notificationHandler = OSMemberFunctionCast(IOServiceMatchingNotificationHandler, this, &LymosTimer::notificationHandler);
...
void LymosTimer::dispatchMessageGated(int* message, void* data)
{
...
void LymosTimer::dispatchMessage(int message, void* data)
{
    if (_notificationServices->getCount() == 0) {
        SYSLOG("brkeys", "No available notification consumer");
        return;
    }
    commandGate->runAction(OSMemberFunctionCast(IOCommandGate::Action, this, &LymosTimer::dispatchMessageGated), &message, data);
}

void LymosTimer::notificationHandlerGated(IOService *newService, IONotifier *notifier)
{
...
bool LymosTimer::notificationHandler(void *refCon, IOService *newService, IONotifier *notifier)
{
    commandGate->runAction(OSMemberFunctionCast(IOCommandGate::Action, this, &LymosTimer::notificationHandlerGated), newService, notifier);
    return true;
}

UInt32 LymosTimer::interfaceID() { return NX_EVS_DEVICE_INTERFACE_ADB; }

UInt32 LymosTimer::maxKeyCodes() { return NX_NUMKEYCODES; }

UInt32 LymosTimer::deviceType() { return 3; }

const unsigned char * LymosTimer::defaultKeymapOfLength(UInt32 * length)
{
    //
