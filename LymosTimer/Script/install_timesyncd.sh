...
# 需要 root
if [[ $EUID -ne 0 ]]; then
  echo "请用 sudo 运行：sudo $0 /path/to/LymosTimeSyncd.c"
  exit 1
fi

SRC="${1:-}"
if [[ -z "$SRC" || ! -f "$SRC" ]]; then
  echo "用法：sudo $0 /path/to/LymosTimeSyncd.c"
  exit 1
fi
...
<dict>
  ...
  <array>
    <string>/usr/local/bin/LymosTimeSyncd</string>
  </array>
  ...
  <key>StandardOutPath</key> <string>/var/log/lymos.timesyncd.out</string>
  <key>StandardErrorPath</key> <string>/var/log/lymos.timesyncd.err</string>
</dict>
</plist>
PLIST
...
echo
echo "验证运行(几秒后)："
echo "  sudo log show --last 3m --predicate 'process == \"LymosTimeSyncd\"' --info"
